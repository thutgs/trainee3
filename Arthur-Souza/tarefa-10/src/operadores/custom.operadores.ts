...
// Acumular contagem de status 
export const statusCount$ = pedidos$.pipe(
  retry(3), // repetindo a validação de erros de pedido aqui para não dar problema caso o pedido falhe
...
// Criamos um tipo para o estado dos entregadores
type EstadoEntregadores = { [id: string]: any };

// aqui usamos o merge para combinar os streams de GPS e Pedidos,
// e o scan para acumular o estado dos entregadores ao longo do tempo, criando um painel consolidado
export const painelEntregador$ = merge(
  gps$.pipe(map(d => ({ ...d, tipo: 'gps' }))),
  pedidos$.pipe(
    retry(3),
  ...
  scan((acc: EstadoEntregadores, curr: any) => {
...
// Criando um stream que mantém os GPS dos últimos 5 segundos
const gpsRecentes$ = gps$.pipe(
  scan((acc: any[], atual: any) => {
...
export const emergencia$ = alertas$.pipe(
  ...
  // juntamos com os GPS recentes para verificar se o entregador do alerta estava acima de 60km/h nos últimos 5 segundos
  withLatestFrom(gpsRecentes$),
  
  // transformamos o alerta em múltiplas emissões (uma para cada GPS infrator)
...
export function logComTimestamp<T>(prefixo: string): MonoTypeOperatorFunction<T> {
  return tap(valor => {
    ...
    // Usamos o prefixo em maiúsculas para destacar no terminal
